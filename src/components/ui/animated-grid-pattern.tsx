...
export function AnimatedGridPattern({
  ...
  const getPos = useCallback((areaWidth: number, areaHeight: number): [number, number] => {
    return [
      Math.floor((Math.random() * areaWidth) / width),
      Math.floor((Math.random() * areaHeight) / height),
    ]
  }, [height, width])

  const generateSquares = useCallback(
    (count: number, areaWidth: number, areaHeight: number) =>
      Array.from({ length: count }, (_, i) => ({
        id: i,
        pos: getPos(areaWidth, areaHeight),
        iteration: 0,
      })),
    [getPos],
  )
